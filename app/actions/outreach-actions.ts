...
export async function sendInvoiceEmailAction(details: InvoiceSendDetails) {
    ...
    if (useWebhook) {
        console.log(`[Email Action] Routing invoice to n8n webhook: ${n8nWebhookUrl}`);
        try {
            ...
            console.error('n8n webhook delivery failed:', webhookErr);
            return { error: `n8n webhook failed: ${webhookErr.message || webhookErr}` };
        }
    } else {
        ...
        try {
            ...
            console.error('Error sending invoice email via SMTP:', err);
            return { error: `Failed to send email via SMTP: ${err.message || err}` };
        }
    }
    ...
    try {
        ...
        console.error('Error updating outreach in DB:', err);
        return { error: `Failed to update database record: ${err.message || err}` };
    }
}
...
export async function sendInvoiceWhatsAppAction(details: InvoiceSendDetails) {
    ...
    try {
        if (useWebhook) {
            console.log(`[WhatsApp Action] Routing invoice to n8n webhook: ${n8nWebhookUrl}`);
            const response = await fetch(n8nWebhookUrl!, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
